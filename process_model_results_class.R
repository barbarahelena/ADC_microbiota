## Processing results subgroups
rm(list=ls())
library(dplyr)
library(ggplot2)
library(ggsci)
library(stringr)
library(tidyverse)
library(svglite)
source("functions.R")

path_true <- 'Amypos_amyneg/output_XGB_class_Amyloidstat_2021_03_15__12-10-58'
path_permuted <- 'Amypos_amyneg/output_XGB_class_Amyloidstat_2021_03_15__12-15-48_PERMUTED'
data_path <- input_path <- 'Amypos_amyneg/input_data' 
labels <- c("Amy+", "Amy-")

compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=15, labels)
plot_features_tests_top(data_path, path_true, top_n=15, nrow=3, labels)


path_true <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__12-19-11'
path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'ADMCI_SCD/input_data'
labels <- c("AD-MCI", "SCD")

compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=15, labels)
plot_features_tests_top(data_path, path_true, top_n=15, nrow=3, labels)

path_true <- 'amyPET/output_XGB_class_amypet_2021_05_04__20-45-12'
#path_permuted <- 'Amypos_amyneg/output_XGB_class_Amyloidstat_2021_03_15__12-15-48_PERMUTED'
data_path <- input_path <- 'amyPET/input_data' 
labels <- c("Amy+", "Amy-")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=15, labels)
plot_features_tests_top(data_path, path_true, top_n=15, nrow=3, labels)


path_true <- 'amystat_pet/output_XGB_class_amyinclPET_2021_04_12__22-52-44'
#path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'amystat_pet/input_data'
labels <- c("Amy+", "Amy-")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=15, labels)
plot_features_tests_top(data_path, path_true, top_n=15, nrow=3, labels)

path_true <- 'amycsf/output_XGB_class_amycsf_2021_03_16__17-21-40'
#path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'amycsf/input_data'
labels <- c("Amy+", "Amy-")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=20, labels)
plot_features_tests_top(data_path, path_true, top_n=20, nrow=4, labels)

path_true <- 'amycsf/output_XGB_class_amycsf_new_2021_07_27__22-52-41'
#path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'amycsf/input_data'
labels <- c("Amy+", "Amy-")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=20, labels)
plot_features_tests_top(data_path, path_true, top_n=20, nrow=4, labels)

path_true <- 'ptau/output_XGB_class_ptau_new_2021_07_27__23-01-26'
#path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'ptau/input_data'
labels <- c("High", "Low")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=20, labels)
plot_features_tests_top(data_path, path_true, top_n=20, nrow=4, labels)

## SCD MCI models

path_true <- 'amycsf_mciscd/output_XGB_class_amycsf_mciscd_2021_08_12__22-15-48'
#path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'amycsf_mciscd/input_data'
labels <- c("Amy+", "Amy-")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=20, labels)
plot_features_tests_top(data_path, path_true, top_n=20, nrow=4, labels)


path_true <- 'ptau_scdmci/output_XGB_class_ptau_scdmci_2021_08_12__20-18-25'
#path_permuted <- 'ADMCI_SCD/output_XGB_class_ADvsSCD_2021_03_15__14-23-51_PERMUTED'
data_path <- 'ptau_scdmci/input_data'
labels <- c("p-tau+", "p-tau-")

#compared_to_permuted_class(path_true, path_permuted)
plot_feature_importance_class(path_true, 15)
plot_features_tests_class(data_path, path_true, top_n=20, labels)
plot_features_tests_top(data_path, path_true, top_n=20, nrow=4, labels)
